...
export default function CartPage() {
  ...
  const handleApplyCoupon = async () => {
    ...
    if (result.success) {
      setCouponSuccess(`Coupon applied! You save ₹${result.discount.toLocaleString()}`);
    } else {
              ...
              <div key={productId} className="flex flex-col sm:flex-row gap-6 p-4 border rounded-lg bg-background">
                ...
                <div className="flex-1 flex flex-col justify-between">
                  <div>
                     ...
                     <p className="font-medium">₹{(item.price || product.price || 0).toLocaleString()}</p>
                  ...
                  <div className="flex items-center gap-4 mt-4">
                       ...
                       <span className="text-sm text-muted-foreground">
                         Line total: ₹{((item.price || 0) * item.quantity).toLocaleString()}
                       </span>
                  </div>
                </div>
              </div>
